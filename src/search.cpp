...
namespace aurora::chess
{
    namespace
    {
        ...
        void score_and_sort_moves(const Board &board, MoveList &moves, Move tt_move = 0)
        {
            ...
            {
                entry.score = static_cast<std::uint16_t>(std::max<Score>(0, move_order_score(board, entry.move, tt_move)));
            }

            std::sort(moves.begin(), moves.end(), [](const MoveEntry &lhs, const MoveEntry &rhs)
    ...
    Searcher::Searcher(const Evaluator &evaluator) noexcept : evaluator_(evaluator) {}

    SearchResult Searcher::search(const Board &board, SearchLimits limits)
    {
        SearchResult result;
        Board working = board;
        ...
        {
            max_quiescence_ply_ = depth + limits.quiescence_depth;
            Move best_move = 0;
            ...
            for (const auto &entry : moves)
            {
                ...
                const Score score = -alpha_beta(working, depth - 1, -kInfiniteScore, -alpha, 1, child_pv);
                working.undo_move();
            ...
            const SearchIteration iteration{
                best_move,
                best_pv,
                moves.empty() ? (working.checkers() != 0 ? -kMateScore : 0) : best_score,
                nodes_,
                depth,
                selective_depth_,
            };
            ...
            {
                table_.store(working.key(), depth, iteration.score, Bound::Exact, iteration.best_move);
            }
        }
    ...
    Score Searcher::alpha_beta(Board &board, std::size_t depth, Score alpha, Score beta, std::size_t ply,
                               std::vector<Move> &pv)
    {
        if (depth == 0)
        {
        ...
        if (const auto *entry = table_.probe(board.key()))
        {
        ...
        for (const auto &entry : moves)
        {
            ...
            std::vector<Move> child_pv;
            const Score score = -alpha_beta(board, depth - 1, -beta, -alpha, ply + 1, child_pv);
            board.undo_move();
        ...
        const Bound bound = best_score <= original_alpha ? Bound::Upper
                           : best_score >= beta         ? Bound::Lower
                                                        : Bound::Exact;
        table_.store(board.key(), depth, best_score, bound, best_move);
        return best_score;
    }

    Score Searcher::quiescence(Board &board, Score alpha, Score beta, std::size_t ply, std::vector<Move> &pv)
    {
        ++nodes_;
        selective_depth_ = std::max(selective_depth_, ply);
        pv.clear();
        ...
        if (!in_check || ply >= max_quiescence_ply_)
        {
            const Score stand_pat = evaluator_.evaluate(board);
            if (stand_pat >= beta)
            {
                return beta;
            }
